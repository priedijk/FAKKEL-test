...
resource "azurerm_key_vault_access_policy" "storageaccount_identity_access_policy" {
  count        = var.secret_data ? 1 : 0
  key_vault_id = azurerm_key_vault.disks.id
  tenant_id    = azurerm_storage_account.file_storage_account.identity[0].tenant_id
  object_id    = azurerm_storage_account.file_storage_account.identity[0].principal_id
...
resource "azurerm_role_assignment" "managed_identity_role" {
  ...
  principal_id         = azurerm_storage_account.file_storage_account.identity.0.principal_id
}


resource "azurerm_storage_account" "file_storage_account" {
  ...
  share_properties {
    dynamic "retention_policy" {
      for_each = var.is_fileshare_soft_delete_enabled == "yes" ? ["true"] : []
      content {
...
resource "azurerm_storage_account_customer_managed_key" "cmk" {
  count              = var.secret_data ? 1 : 0
  key_vault_id       = azurerm_key_vault.disks.id
  storage_account_id = azurerm_storage_account.file_storage_account.id
...
resource "azurerm_key_vault_secret" "primary_access_key" {
  ...
  value        = sensitive(azurerm_storage_account.file_storage_account.primary_access_key)
  key_vault_id = azurerm_key_vault.disks.id
