...
# output "blob_names" {
#   description = "List of Storage blob container names"
#   value = { for blob in azurerm_storage_container.blob_containers : blob.name => {
#     id   = blob.id
#     name = blob.name
#     }
...
output "blob" {
  description = "List of Storage blob outputs"
  value = {
    primary_endpoint   = azurerm_storage_account.storage_account.primary_blob_endpoint
    secondary_endpoint = azurerm_storage_account.storage_account.secondary_blob_endpoint
    # private_endpoint_id =
    blobs = { for blob in azurerm_storage_container.blob_containers : blob.name => {
      id   = blob.id
      name = blob.name
      }
    }
...
output "fileshare" {
  description = "List of Storage blob outputs"
  value = {
    fileshares = { for fileshare in azurerm_storage_share.fileshares : fileshare.name => {
      id   = fileshare.id
      name = fileshare.name
      }
    }
